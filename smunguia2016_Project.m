...
%% Part 1:load and inspect untrained data
%Loading the training data
categories = {'cat', 'dog'};

rootFolder = '\Users\vigil\Documents\MATLAB\dog_vs_cat\train';
...
rootFolder = '\Users\vigil\Documents\MATLAB\dog_vs_cat\test1';
...
cnnFolder = '\Users\vigil\Documents\MATLAB\cats_dogs_starter\networks';
...
dataFolder = '\Users\vigil\Documents\MATLAB\cats_dogs_starter\data\PetImages';
categories = {'cat', 'dog'};
...
%% 2.3: Divide data into training and testing sets // changed from , cvSet & 0.3
[trainingSet, testSet] = splitEachLabel(imds, 0.3, 'randomize');
% 60/20/20 set [trainingSet, testSet, cvSet] = splitEachLabel(imds, 0.6, 0.2, 'randomize');
...
fprintf('\t Multi-Class SVM \n')
confMat = confMat./sum(confMat,2);
mean(diag(confMat))

%% 5.3: Test SVM classifier on an unseen image
newImage = '\Users\vigil\Documents\MATLAB\cats_dogs_starter\Siamese.jpeg'; % any cat or dog image should do!
...
%% Part 4: Train a Tree classifier
% Get training labels from the trainingSet
trainingLabels = trainingSet.Labels;
t2 = templateTree('Surrogate','on');
tree = fitcecoc(trainingFeatures, trainingLabels, ...
    'Learners', t2, 'Coding', 'onevsall', 'ObservationsIn', 'columns');
...
%% 5.2: Test Tree classifier's prediction accuracy and produce confusion matrix
% Pass CNN image features to trained classifier
...
% Convert confusion matrix into percentage form
confMat = bsxfun(@rdivide,confMat,sum(confMat,2)); 
fprintf('\t Tree Classifier \n')
confMat = confMat./sum(confMat,2);
mean(diag(confMat))

%% 5.3: Test Tree Classifier on an unseen image
newImage = '\Users\vigil\Documents\MATLAB\cats_dogs_starter\Siamese.jpeg'; % any cat or dog image should do!
img = readAndPreprocessImage(newImage);
imageFeatures = activations(convnet, img, featureLayer);
label = predict(tree, imageFeatures);

% Display test image and assigned label
figure, imshow(img), title(char(label)); 

%% Part 4: Train a naive bayes classifier
% Get training labels from the trainingSet
trainingLabels = trainingSet.Labels;
t3 = templateNaiveBayes();
nb = fitcecoc(trainingFeatures, trainingLabels, ...
    'Learners', t3 , 'Coding', 'onevsall', 'ObservationsIn', 'columns');
...
%% 5.3: Test a NB on an unseen image
newImage = '\Users\vigil\Documents\MATLAB\cats_dogs_starter\Siamese.jpeg'; % any cat or dog image should do!
img = readAndPreprocessImage(newImage);
imageFeatures = activations(convnet, img, featureLayer);
label = predict(nb, imageFeatures);

% Display test image and assigned label
figure, imshow(img), title(char(label)); 
%% Part 4: Train a K-nearest classifier
% Get training labels from the trainingSet
trainingLabels = trainingSet.Labels;
t4 = templateKNN('NumNeighbors',5,'Standardize',1);
k = fitcecoc(trainingFeatures, trainingLabels, ...
    'Learners', t4, 'Coding', 'onevsall', 'ObservationsIn', 'columns');
...
%% 5.2: Test classifier's prediction accuracy and produce confusion matrix
% Pass CNN image features to trained classifier
predictedLabels = predict(k, testFeatures);
...
%% 5.3: Test K-nearest on an unseen image
newImage = '\Users\vigil\Documents\MATLAB\cats_dogs_starter\Bulldog.jpg'; % any cat or dog image should do!
img = readAndPreprocessImage(newImage);
imageFeatures = activations(convnet, img, featureLayer);
label = predict(k, imageFeatures);
